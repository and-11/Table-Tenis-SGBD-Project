--S? se creeze un subprogram stocat independent care, pentru un turneu dat ca parametru, s? determine juc?torii participan?i ?i s? analizeze sponsoriz?rile acestora. Pentru fiecare juc?tor se vor re?ine sponsorii s?i (într-un VARRAY), valorile sponsoriz?rilor (într-un tabel imbricat) ?i suma total? a sponsoriz?rilor (într-un tabel indexat). Subprogramul va afi?a informa?iile ob?inute.
--ex 6
...
CREATE OR REPLACE PROCEDURE analiza_sponsorizari_turneu (
    ...
    -- 3. Associative array: total sponsorizari per jucator
